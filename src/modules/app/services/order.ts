...
export class OrderService {
  ...
  public async remove(oderId: number, currentOrder: IOrder): Promise<void> {
    const order = await this.orderRepository.findById(oderId);
    ...
    if (order.id === currentOrder.id) {
      throw new BadRequestException('not-allowed-remove-order');
    }

    return this.orderRepository.remove(oderId);
  }
